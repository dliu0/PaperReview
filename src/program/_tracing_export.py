...
This is the CodeEvolver tracing skill's ``assets/file_exporter.py``,
copied in verbatim. It is a DEVELOPMENT tool and nothing in the seed
program imports it: inside CodeEvolver the orchestrator already collects
spans and writes ``/traces/iteration_{N}_{suffix}/``, so installing this
there would be redundant. Its one job is to make ``@traceable`` output
visible when the pipeline runs OUTSIDE the orchestrator, where OTel
installs a no-op provider and spans would otherwise evaporate.

It needs ``opentelemetry-sdk``, which is deliberately NOT in
``requirements.txt`` -- that file builds the fleet image and the engine
installs the SDK into every run's venv itself. Install it locally instead:

    pip install opentelemetry-sdk

Usage
-----
    from scripts.trace_one import ...   # see scripts/trace_one.py
