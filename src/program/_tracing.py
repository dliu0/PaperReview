...
This is the CodeEvolver tracing skill's `assets/traceable.py`, copied in
verbatim, plus a short "Local additions" section at the bottom for the one
case the decorator cannot express (see there for why).
...
# --- Local additions ---------------------------------------------------------
# Everything above is the skill asset, unmodified. Below are two helpers this
# repository needs; keeping them here rather than in `review_pipeline.py` keeps
# the program file about reviewing papers.


@contextlib.contextmanager
def span(name: str, kind: str):
    """A hand-rolled span, for the one call `@traceable` cannot wrap.

    `@traceable` is the right tool for a function whose span is exactly
    "inputs in, output out". `ReviewPipeline._complete` is not that function:

      - Its `messages` argument carries the whole paper (~55k chars). The
        decorator records every declared parameter, and its only lever is
        `max_attr_chars`, which applies to inputs *and* output alike — so
        bounding the prompt would also truncate the response, which is the
        one thing the trace exists to capture in full.
      - Its most interesting facts (attempts, stream chunks, the errors of
        retries that later succeeded) are produced *inside* the retry loop.
        A wrapper only sees the call go in and the value come out, so a
        recovered hang is invisible to it.

    The skill sanctions exactly this: use `tracer.start_as_current_span(...)`
    directly for a custom case, and set the same `ce.*` attributes so the
    converter needs no special handling.

    Yields a setter `(key, value)` that writes a `ce.*` attribute, so callers
    need no OTel-vs-no-op branching. Never raises: tracing must not fail a row.
    """
    if not _OTEL_AVAILABLE:
    ...
    with tracer.start_as_current_span(name) as sp:
        sp.set_attribute("ce.span_kind", kind)
...
def summarize_messages(messages, excerpt_chars: int = 1500) -> str:
    """`[{role, chars, content}]` as JSON, each content bounded.

    The row's full input is already in `row.json` and the harness truncates
    the root span's copy, so repeating a ~50k-token paper inside a span costs
    trace size for no new information. Recording role + true length + a
    bounded excerpt keeps the prompt's shape readable at a fraction of the
    size, and stays generic over the message list so it survives the review
    ...
    for m in messages or []:
        ...
        if len(content) > excerpt_chars:
            excerpt += (
                f"\n... [{len(content) - excerpt_chars} more chars elided; "
                f"the row's full input is in row.json]"
            )
        out.append({"role": m.get("role"), "chars": len(content), "content": excerpt})
    return json.dumps(out, indent=2)


__all__ = ["traceable", "span", "summarize_messages"]
